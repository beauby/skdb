...
message, along with a signature of the `Auth` nonced message using the
private key, in order to authenticate the mirroring session. The
private key never leaves the client[fn:5].
...
The ~create_reactive_request()~ function of the [[*L][Low Level API]] takes a
public key as its last parameter. The generated response table is
readable only by authenticating using the same public/private key
pair.
