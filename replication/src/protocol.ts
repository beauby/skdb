...
function encodeAuthMsg(msg: ProtoAuth): ArrayBuffer {
  ...
  const buf = new ArrayBuffer(117 + msg.clientVersion.length);
  const uint8View = new Uint8Array(buf);
  const dataView = new DataView(buf);
  dataView.setUint8(0, 0x0); // type
  dataView.setUint8(1, 0x0); // version
...
function encodeDataMsg(msg: ProtoData): ArrayBuffer {
  ...
  dataView.setUint8(9, msg.collection.length);
  textEncoder.encodeInto(
    msg.collection,
    uint8View.subarray(10, 10 + msg.collection.length),
  );
  const encodePayload = textEncoder.encodeInto(
    msg.payload,
    uint8View.subarray(10 + msg.collection.length),
  );
  return buf.slice(
    0,
    10 + msg.collection.length + (encodePayload.written || 0),
  );
}
...
function encodeRequestTailBatchMsg(msg: ProtoRequestTailBatch): ArrayBuffer {
...
function decodeAuthMsg(bytes: ArrayBuffer): ProtoAuth {
  ...
  const signature = uint8View.subarray(85, 117);
  const clientVersion = textDecoder.decode(uint8View.subarray(117));
  return {
    type: "auth",
    pubkey,
    nonce,
...
function decodeDataMsg(bytes: ArrayBuffer): ProtoData {
  ...
  const collectionLen = dv.getUint8(9);
  const collection = textDecoder.decode(
    uint8View.subarray(10, 10 + collectionLen),
  );
  const payload = textDecoder.decode(uint8View.subarray(10 + collectionLen));
  return {
    type: "data",
    tick,
    collection,
...
function decodeRequestTailBatchMsg(bytes: ArrayBuffer): ProtoRequestTailBatch {
  throw new Error("TODO");
}
